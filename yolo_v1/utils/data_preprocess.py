...
for year, image_set in sets:
    if not os.path.exists("data/VOCdevkit/VOC%s/labels/" % (year)):
        os.makedirs("data/VOCdevkit/VOC%s/labels/" % (year), exist_ok=True)
    image_ids = (
        open("data/VOCdevkit/VOC%s/ImageSets/Main/%s.txt" % (year, image_set))
        .read()
        .strip()
        .split()
    )
    ...
    for image_id in image_ids:
        list_file.write(
            "%s/data/VOCdevkit/VOC%s/JPEGImages/%s.jpg\n" % (wd, year, image_id)
        )
